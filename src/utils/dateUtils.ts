...
export interface CountryGreetingData {
  greetingText: string;
  emoji: string;
  userName: string;
  fullGreeting: string;
...
export function getCountryGreetingParts(
  userName: string,
  ...
  return {
    ...
    emoji,
    userName: name,
    fullGreeting: `${greetingText} ${emoji}, ${name} 👋`,
  };
}
...
export function getCountryTimeOfDayGreeting(
  ...
  return getCountryGreetingParts(userName, countryNameOrCode, fallbackTimezone, date).fullGreeting;
}
